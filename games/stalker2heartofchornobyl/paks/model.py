...
class S2HoCPaksColumns(IntEnum):
    PRIORITY = auto()
    PAK_NAME = auto()
    SOURCE = auto()


class S2HoCPaksModel(QAbstractItemModel):
    ...
    def headerData(
        ...
        column = S2HoCPaksColumns(section + 1)
        match column:
            case S2HoCPaksColumns.PAK_NAME:
                return "PAK Name"
            case S2HoCPaksColumns.PRIORITY:
                return "Priority"
            case S2HoCPaksColumns.SOURCE:
                return "Source"
    ...
    def data(self, index: QModelIndex, role: int = Qt.ItemDataRole.DisplayRole) -> Any:
        if not index.isValid():
            return None
        if index.column() + 1 == S2HoCPaksColumns.PAK_NAME:
            if role == Qt.ItemDataRole.DisplayRole:
                return self.paks[index.row()][0]
        elif index.column() + 1 == S2HoCPaksColumns.PRIORITY:
            if role == Qt.ItemDataRole.DisplayRole:
                return index.row()
        elif index.column() + 1 == S2HoCPaksColumns.SOURCE:
            if role == Qt.ItemDataRole.DisplayRole:
