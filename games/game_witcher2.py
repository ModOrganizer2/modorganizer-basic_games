...
class Witcher2SaveGame(BasicGameSaveGame):
    def allFiles(self):
        return [self._filename, self._filename.replace(".sav", "_640x360.bmp")]
...
class Witcher2Game(BasicGame):
    ...
    def init(self, organizer: mobase.IOrganizer):
        ...
        self._featureMap[mobase.SaveGameInfo] = BasicGameSaveGameInfo(
            lambda s: Path(str(s.parent) + "\\" + s.stem + "_640x360").with_suffix(".bmp")
        )
        return True

    def iniFiles(self):
        return ["User.ini", "Rendering.ini", "Community.ini", "UserContent.ini", "DIMapping.ini", "Input_QWERTY.ini", "Input_AZERTY.ini", "Input_QWERTZ.ini"]
